...
def set_sdk_variables() {
    ...
    if (SPEC.contains('zos')) {
        SDK_FILENAME = buildspec_manager.getSpec(SPEC).getScalarField("misc.sdk_filename_template", SDK_VERSION)
        echo "Inside the zos check for filename ... ${SDK_FILENAME}"
    } else {
